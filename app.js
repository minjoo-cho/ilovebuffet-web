...
export default function App() {
    ...
    const tickerMap = {
        '애플': 'AAPL', '마이크로소프트': 'MSFT', '구글': 'GOOGL', '아마존': 'AMZN', '테슬라': 'TSLA',
        '엔비디아': 'NVDA', '비자': 'V', '제이피모간체이스': 'JPM', '유나이티드헬스그룹': 'UNH', '존슨앤드존슨': 'JNJ'
    };
    ...
    const mockStockSearch = (query) => {
        ...
        for (const key in tickerMap) {
            if (tickerMap[key] === normalizedQuery) {
                return mockStockData[tickerMap[key]];
            }
        }
        return null;
    };
